...
export default {
  ...
  methods: {
    sendEvent() {
      if (
        this.$store.getters.getStep < 3 &&
        !this.$store.getters.isCoDefendant2Valid
      ) {
        this.dialogData.modalHeader = "Error!";
        this.dialogData.modalBody1 = `${this.desc[2]} form not valid`;
        this.dialogData.modalBody2 = `Complete ${this.desc[2]} form`;
        this.dialogData.modalBody3 = "And, Kool is the rool";

        this.$emit("show-dialog", this.dialogData);
        return;
      }
      if (this.$store.getters.isCoDefendant2Valid) {
        this.$emit("change-form", 3);
        return;
      } else {
        this.dialogData.modalHeader = "Can't Do";
        this.dialogData.modalBody1 = "3rd Party 2 form not valid";
