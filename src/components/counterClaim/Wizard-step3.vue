...
<script>
import Quantum from "@/components/Quantum-cc";
...
export default {
  ...
  methods: {
    async updateFileUpload(e) {
      if (e.delFile) {
        var url = `quantafilesCC/${JSON.stringify(e.id)}`;
        this.$store
          ...
          .catch(err => {
            console.log("ccQuantum file not deleted " + err);
          });
      }
      var varQuanta = {};
      ...
      try {
                ...
                if (arr[i].filename === filename) {
                  if (!arr[i].id) {
                    ...
                    var file = {
                      ...
                      ccQuantumId: arr[i].ccQuantumId
                    };
                    try {
                      this.delIncoming.push(file);
                      var fileObj = {
                        url: `quantafilesCC/${file.id}`,
                        file: file
                      };
                      await this.$store.dispatch("updateFile", fileObj);
                    } catch (err) {
                      console.log("ccQuantum file not updated " + err);
                    }
                  }
                  break;
                }
        ...
        varQuanta.quantaId = this.$store.getters.getCcQuantaId;
    ...
    loadMeritFiles(fileArr, varQuanta) {
      ...
      fileArr.forEach(file => {
        retArr.push({
          ...
          ccQuantumId: varQuanta.quantaId
        });
      });
      return retArr;
    }
  ...
  async beforeDestroy() {
    ...
    if (currentFiles != null && currentFiles.length > 0) {
      await this.$store.dispatch("doCcQuantaFiles", currentFiles);
    }
  ...
  async created() {
    ...
    if (this.$store.getters.getCcQuantaFiles) {
      var files = this.$store.getters.getCcQuantaFiles;
